...
/**
 * Stable empty fallback. Zustand v5 selectors run through useSyncExternalStore,
 * which throws "Maximum update depth exceeded" if the selector returns a fresh
 * reference each render. Returning a shared constant for the (common) no-checkins
 * case keeps the snapshot stable.
 */
const EMPTY_STATUSES: Record<string, BeatStatus> = {};
...
export function useTodayStatuses(): Record<string, BeatStatus> {
  return useBeat((s) => s.byDate[todayKey()] ?? EMPTY_STATUSES);
}
